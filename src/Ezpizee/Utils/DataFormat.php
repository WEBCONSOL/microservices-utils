...
class DataFormat
{
    ...
    public static function validTelephone(string $telephone, string $countryCode=null): string {
        $properties = self::phoneProperties($telephone, $countryCode);
        return $properties['telephone'];
    }

    public static function phoneProperties(string $telephone, string $countryCode=null): array {
    ...
    public static function numberOnly(string $str): string {return preg_replace("/[^0-9]/", "", $str);}

    public static function isEzpzFakePhone(string $phone): bool {return StringUtil::startsWith($phone, Constants::EZPZ_FAKE_PHONE_PFX);}
    public static function isEzpzFakeEmail(string $email): bool {return StringUtil::startsWith($email, Constants::EZPZ_FAKE_EMAIL_PFX);}
}