...
final class RequestBodyValidator
{
    ...
    public static function throwError(ListModel $field): void {
        if (DEBUG) {CustomResponse::setDebugInfo($field->getAsArray());}
        throw new RuntimeException(ResponseCodes::MESSAGE_ERROR_INVALID_FIELD, ResponseCodes::CODE_ERROR_INVALID_FIELD);
    }

    public static function validate(ListModel $field, $v): void {
    ...
    public static function validateFile(ListModel $field): void {
        if (!(isset($_FILES) && is_array($_FILES) && sizeof($_FILES) && isset($_FILES[$field->get('name', '')]))) {
            self::throwError($field);
        }
    }

    public static function validateArray(ListModel $field, $v): void {
    ...
    public static function validateMixed(ListModel $field, $v): void {
        if (!is_null($v)) {
            if (is_numeric($v)) {
                self::validateNumber($field, $v);
            }
    ...
    public static function validateURL(ListModel $field, string $v): void {
        if ($field->get('size', 0)) {
            if (!strlen($v)) {
                self::throwError($field);
            }
    ...
    public static function validateEmail(ListModel $field, string $v): void {
        if ($field->get('size', 0)) {
            if (!strlen($v)) {
                self::throwError($field);
            }
    ...
    public static function validateBoolean(ListModel $field, $v): void {
        if ($v !== 'true' && $v !== true && $v !== 'false' && $v !== false) {
            self::throwError($field);
        }
    }

    public static function validateNumber(ListModel $field, string $v): void {
    ...
    public static function validateClientCredentials(ListModel $field, string $v): void {
        if ($field->get('size', 0)) {
            if (!strlen($v)) {
                self::throwError($field);
            }
    ...
    public static function validateString(ListModel $field, string $v): void {
        if ($field->get('size', 0)) {
            if (!strlen($v)) {
                self::throwError($field);
            }
    ...
    private static function validateSize($size, $num): bool {
